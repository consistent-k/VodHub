...
import type { Namespace, HomeVodData } from '@/types';
...
export const handler = async (ctx: Context, namespace: Namespace) => {
    try {
        ...
        if (code === 1) {
            const vod_list: HomeVodData[] = list.map((item) => ({
                vod_id: item.vod_id,
        ...
        logger.error(`${HOME_VOD_MESSAGE.ERROR} - ${namespace.name} - ${error instanceof Error ? error.message : String(error)}`);
        return {
